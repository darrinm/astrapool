...
## Earlier drafts and launch render

The `astra-pool-launch-trailer.mp4` and `astra-pool-action-trailer*.mp4` files preserve earlier edits. The approved final remains `astra-pool-trailer.mp4`.

The root-level `capture.html`, `capture.js`, `server.mjs`, and `compose.py` are the earlier launch-trailer pipeline. Its `render/` directory preserves 900 captured frames, contact sheets, stills, the capture timeline, and soundtrack. These are historical source materials, not inputs to the current action-trailer rebuild described above.

`action/README.md` preserves notes from the earlier action cut, and `action/search.mjs` preserves its standalone shot-search experiment. Use this README for the current final-trailer workflow.
