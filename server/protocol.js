...
export const initialSnapshot = (breaker = 0, wins = [0, 0]) => ({ match: newMatch(breaker, wins), balls: rackPositions() });
...
export function placeCue(snapshot, position) {
  ...
  return { match: { ...snapshot.match, ballInHand: false, message: `Player ${snapshot.match.turn + 1}: cue ball placed. Take your shot.` },
